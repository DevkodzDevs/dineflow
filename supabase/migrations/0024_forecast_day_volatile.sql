-- ═══════════ The Tomorrow brief could never be built ═══════════
-- forecast_day gathers eight weeks of the same weekday into a temporary table. Postgres only
-- permits CREATE TABLE inside a VOLATILE function, and the copy living in the database was the
-- STABLE one from 0006, so every visit to Tomorrow answered:
--     "Tomorrow's brief could not be built: CREATE TABLE is not allowed in a non-volatile function"
--
-- 0007 already rewrote the function to be volatile, but the database never received that rewrite:
-- scripts/migrate.mjs records each file by name and skips it forever after, so the edit made to
-- 0007 after it had been applied only ever existed on disk. Same trap as 0023. The correction has
-- to arrive as a new file, and the volatility is now spelled out instead of left to the default.
