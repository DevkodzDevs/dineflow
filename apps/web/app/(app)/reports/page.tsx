...
export default async function ReportsPage({ searchParams }: { searchParams: Promise<{ days?: string }> }) {
   ...
   */
  const [{ data: summary }, { data: closes }, { data: kitchen }, { data: rooms }, { data: audits }] = await Promise.all([
  ...
  return (
    <>
      ...
      <ReportsClient days={n} summary={(summary ?? {}) as Summary} closes={closes ?? []} kitchen={(kitchen ?? null) as KitchenSpeed | null} hotel={(rooms ?? null) as HotelKpis | null} audits={(audits ?? []) as AuditRow[]} />
    </>
  );
}
